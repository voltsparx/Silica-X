...
def ensure_module_catalog(
    *,
    ...
    expected_source_root = Path(source_root).as_posix()
    payload_source_root = str(payload.get("source_root") or "").strip()
    if payload_source_root and payload_source_root != expected_source_root:
        return build_module_catalog(source_root, output_root=output_root, max_workers=max_workers)
